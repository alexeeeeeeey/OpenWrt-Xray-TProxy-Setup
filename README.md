...
В веб-интерфейсе можно вставить сразу несколько ссылок — по одной на строку. Если вставить URL подписки, все поддерживаемые ноды будут добавлены отдельными профилями и сгруппированы по источнику. Каждую подписку можно обновить отдельно, удалить целиком вместе со всеми её нодами или обновить все подписки сразу. Кнопка «Пинг» в карточке профиля измеряет ICMP-задержку до его сервера, не переключая активное подключение.
