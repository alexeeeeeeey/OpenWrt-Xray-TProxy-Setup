...
```sh
opkg update && opkg install kmod-nft-tproxy kmod-nf-tproxy unzip uclient-fetch ca-bundle ca-certificates openssl-util coreutils-base64 jsonfilter uhttpd && uclient-fetch -q -O - https://raw.githubusercontent.com/alexeeeeeeey/OpenWrt-Xray-TProxy-Setup/main/install.sh | sh
```
...
В веб-интерфейсе можно вставить сразу несколько ссылок — по одной на строку. Поддерживаются обычные/base64 подписки, одиночные JSON-конфигурации Xray и JSON-массивы полных конфигураций. Для JSON сохраняются собственные `outbounds`, `routing.rules`, `routing.balancers`, `observatory` и `burstObservatory`; каждый элемент массива становится отдельным профилем. Загрузка подписок всегда выполняется напрямую, с заголовками совместимого клиента и без переменных proxy, поэтому сломанный активный профиль не мешает обновлению.

Официальные `geosite.dat` и `geoip.dat` устанавливаются вместе с Xray. Они нужны для правил вроде `geosite:category-ru` и `geoip:private`; пустые или отсутствующие базы автоматически восстанавливаются при повторном запуске установщика.

Кнопка «Проверить» запускает изолированный временный SOCKS-вход для выбранного профиля и делает через него HTTPS-запрос к `https://www.gstatic.com/generate_204`. Показанное время отражает реальное прохождение трафика через профиль, а не ICMP ping до сервера.

Проверочные Xray-конфигурации заранее генерируются в `/etc/xray-manager/generated` при добавлении или обновлении подписки и проходят `xray run -test`. Обновление набора выполняется атомарно. Поэтому кнопка «Проверить» только запускает уже проверенный конфиг, связанный с выбранным профилем, на локальном SOCKS-порту `19090`.
...
xray-manager ping-link <id>
xray-manager del-subscription 'https://example.com/subscription'
```
...
- Включён: менеджер ставит локальный DNS через Xray первым, а штатные WAN DNS оставляет следующими в строгом порядке. Если локальный DNS/Xray не отвечает, `dnsmasq` автоматически переходит на обычную сеть.
...
## Автообновление подписок и менеджера

Интервал можно выбрать в веб-интерфейсе или задать из CLI. Значение `0` выключает расписание:

```sh
xray-manager set-auto-refresh 6
```

После обновления остаётся активным прежний профиль, если он всё ещё есть; иначе выбирается первый доступный профиль подписки. Обновление менеджера из основного репозитория запускается кнопкой «Обновить менеджер» или командой `xray-manager self-update`.
